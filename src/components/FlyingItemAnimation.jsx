...
const FlyingItemAnimation = ({
  ...
  const [position, setPosition] = useState(
    startPosition || { x: window.innerWidth / 2, y: window.innerHeight / 3 }
  );
  const dimensions = { width: 60, height: 90 };
  ...
  useEffect(() => {
    if (!isAnimating) {
      ...
      const animate = () => {
        ...
        // Uppdatera position
        const newX =
          position.x + (targetPosition.x - position.x) * easedProgress;
        const newY =
          position.y + (targetPosition.y - position.y) * easedProgress;
  ...
  }, [getDefaultTarget, isAnimating, targetType, position, onComplete]);
