...
const MovieDetailModal = ({ movie, onClose }) => {
  ...
  const mockMovie = {
    ...
    runtime: 128,
    overview: "This is a sample overview of the movie. It describes the plot, characters, and what makes this movie interesting to watch.",
    credits: {
      cast: [
    ...
    >
      ...
      >
        ...
        <div className="relative z-10 flex justify-center mb-4 -mt-16">
          <img
            src={displayMovie.poster_path ? `${IMAGE_BASE_URL}${displayMovie.poster_path}` : "/no-profile.png"}
        ...
        <div className="px-6 mt-2 text-left">
          <h2 className="mb-1 text-3xl font-bold text-yellow-400">{displayMovie.title}</h2>
          {displayMovie.tagline && (
            <div className="mb-2 text-lg italic text-yellow-300">"{displayMovie.tagline}"</div>
          )}
          <div className="flex items-center gap-2 mb-2">
            <span className="inline-block px-2 py-1 text-sm font-semibold text-white bg-yellow-600 rounded">{displayMovie.vote_average?.toFixed(2)} / 10</span>
            <span className="text-sm text-gray-200">{displayMovie.vote_count} votes</span>
          </div>
          <div className="mb-2 text-base font-medium text-gray-200">
            {displayMovie.genres && displayMovie.genres.map(g => g.name).join(", ")}
          </div>
        ...
        <div className="p-6 pt-3 pb-28">
          ...
          {/* Trailer */}
          {displayMovie.videos && displayMovie.videos.results && displayMovie.videos.results.length > 0 && (
            <div className="mb-4">
